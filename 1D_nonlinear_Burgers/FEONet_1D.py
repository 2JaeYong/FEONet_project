...
# ARGS
parser = argparse.ArgumentParser("1D_nonlinear_Burgers")
...
def weak_form(coeff_u, load_vec_f, stiff):
    # pts : N+1
    # num of basis : N+1
    # coeff_u : (num_f, 1, N+1)
    # 
    ...
    ## LHS
    LHS = 0.1*stiff*coeff_u
    LHS=torch.sum(LHS,dim=-1)
    
    pad=nn.ConstantPad1d(1, 0)
    uux=torch.zeros(coeff_u[:,0,:].shape).cuda()
    pad_exact_u=pad(coeff_u[:,0,:])
    for i in range(coeff_u.shape[-1]):
        uux[:,i]=(-pad_exact_u[:,i]+pad_exact_u[:,i+2])*(pad_exact_u[:,i]+pad_exact_u[:,i+1]+pad_exact_u[:,i+2])
    uux*=1/6
    uux[...,0]=0
    uux[...,-1]=0
    LHS+=-uux
...
def closure(model, f_value, load_vec_f, stiff):
    pred_coeff_u = model(f_value)
    LHS, RHS = weak_form(pred_coeff_u, load_vec_f, stiff)
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch_idx, sample_batch in enumerate(trainloader):
        ...
        loss,u_pred = closure(model, f_value, load_vec_f, STIFF)
        ...
        with torch.no_grad():
            model.eval()
            _,u_pred = closure(model, f_value, load_vec_f, STIFF)
